...
import com.robinye.mbp.client.UserFeignClient;
...
public class MbpVideoServiceImpl extends ServiceImpl<MbpVideoMapper, MbpVideo> implements IMbpVideoService {
    ...
    public MbpVideoVO getVideo(Long id) {
        ...
        MbpVideoVO mbpVideoVO = mbpVideoConvertor.toVO(mbpVideo);

        Long userId = mbpVideo.getUserId();
        if (userId != null) {
            MbpUserVO mbpUserVO = userFeignClient.getUserVO(userId);
            mbpVideoVO.setMbpUserVO(mbpUserVO);
        }
