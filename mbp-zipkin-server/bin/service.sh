...
if [[ "$1" == "start" ]]; then
    ...
    if [[ ! -f "$PID_FILE" ]]; then
        ...
        nohup KAFKA_BOOTSTRAP_SERVERS=${KAFKA_BOOTSTRAP_SERVERS} ES_HOSTS=${ES_HOSTS} java ${JAVA_OPTIONS} -jar ${APP_HOME}/lib/mbp-video-${APP_VERSION}.jar 1>> ${ACCESS_LOG} 2>> ${ERROR_LOG} 2>&1 &echo $! > ${PID_FILE}
